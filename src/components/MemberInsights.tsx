...
import { Trophy, Flame, Dumbbell, ArrowRight, CheckCircle2, Zap, Globe2, Target } from 'lucide-react';
...
export default function MemberInsights() {
  ...
  useEffect(() => {
    const fetchAnalytics = async () => {
      try {
        const res = await api.get(`/memberships/dashboard-analytics`);
        if (res.data?.data) {
          setData(res.data.data);
        }
        const planRes = await api.get(`/ai/todays-focus/detail?generate=true`);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {/* Activity Heatmap */}
        <ConsistencyHeatmap heatmapData={heatmap} loading={loading} />
      </div>
    </div>
  );
}
