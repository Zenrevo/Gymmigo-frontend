...
const EMPTY_HEATMAP_DAYS: HeatmapDay[] = Array.from({ length: 28 }, () => ({ intensity: 0 }));
...
export default function ConsistencyHeatmap({ heatmapData, loading }: ConsistencyHeatmapProps) {
  ...
  const days = heatmapData && heatmapData.length > 0 ? heatmapData : EMPTY_HEATMAP_DAYS;
  const activeDays = days.filter((day) => Number(day.count ?? day.intensity ?? 0) > 0).length;
  ...
  return (
    <div className="glass-card p-6 md:p-8 space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        {days.map((day, i) => {
          const intensity = Number(day.intensity ?? day.count ?? 0);
          return (
            <motion.div
              key={day.date || i}
              ...
              className={clsx(
                ...
                intensity === 0 ? "bg-white/5 border-white/5 hover:border-white/20" :
                intensity === 1 ? "bg-primary/30 border-primary/20 hover:border-primary/50" :
                intensity === 2 ? "bg-primary/60 border-primary/40 hover:border-primary/80" :
